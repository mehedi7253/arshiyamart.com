                   ...
                   @foreach ($viewpayment as $pay)
                    <tr class="gradeX">
                      <td>{{ $i++ }}</td>
                       ...
                       <td class="center">
                           @if($pay->status!=2)
                            <a href="{{ url('/admin/app_pay_g/'.$pay->id) }}" style="padding:8px; background:green; color:white"> Approved</a>
                            @else
                             OK
...
Upline: {{ $pay->up_name}}
                      ...
                      <td class="center">
                            @if($pay->status > 20)
                         <a <?php /*id="delproduct" href="{{ url('/admin/delete-product/'.$product->id) }}" */?> rel="{{ $pay->id }}" rel1="delete_pay" href="javascript:" class="btn btn-danger btn-mini del">--</a>
                            @else
--
