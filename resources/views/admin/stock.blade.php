...
<h4><span style="color:blue">Filter by:</span>
<a href="?t=1">  Offer </a>|<a href="?t=2">  Campaing</a> | <a href="?t=4"> Stock > 0</a> | <a href="?f=3"> View All </a>
                  ...
                  ?>
                   @foreach ($products2 as $product)
                    <tr class="gradeX">
                      ...
                      <td>Code: {{ $product->id }} <br> Style: {{$product->product_code}}<br>{{ $product->product_color }}<br>BDT: {{ $product->price }}  
...
<td>
   <?php 
   $sale_count0=DB::table('orders_products')->where('product_id',$pppid)->whereNULL('d_status')->sum('product_qty');
   $sale_count1=DB::table('orders_products')->where('product_id',$pppid)->where('d_status',1)->sum('product_qty');
   $sale_count11=DB::table('orders_products')->where('product_id',$pppid)->where('d_status',11)->sum('product_qty');
   $sale_count2=DB::table('orders_products')->where('product_id',$pppid)->where('d_status',2)->sum('product_qty');
   $sale_count3=DB::table('orders_products')->where('product_id',$pppid)->where('d_status',3)->sum('product_qty');
   $sale_count4=DB::table('orders_products')->where('product_id',$pppid)->where('d_status',4)->sum('product_qty');
   ?>
   
   <b><span style="color:green">  Online Sale: {{$sale_count1}}<br>
     Direct Sale: {{$sale_count11}}<br>
     </span>
   </b>  <span style="color:black">  
   In Process: {{$sale_count0}}<br> </span>
  
  <span style="color:green">  
   Confirm: {{$sale_count4}}<br> </span>
  
  
    <span style="color:blue"> 
    
    
   Shipping: {{$sale_count2}}<br>
   </span>
    <span style="color:red"> 
    Cancelled: {{$sale_count3}}
    </span>
                ...
                <td>
                    ...
                    @endif
                     ...
                     <a href="{{ url('/admin/add-attribute/') }}/{{$product->id}}"  target="_blank" class="btn btn-danger btn-mini" style="background:red; color:white"> Stock Manage  </a>
