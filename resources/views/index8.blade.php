     ...
     @else
      ...
      ?>
                             ...
                             <a href="{{ url('/product/'.$product->id) }}" style="color:white; font-size:65%"><span class="fa fa-eye"></span> 
                              ...
                              <div class="" id="newsc{{$product->id}}" onclick="newappppc{{$product->id}}()" type="button" style="cursor: alias">
                              <div class=""  id="newloc{{$product->id}}" style="background:{{$bg_t_c}}; text-align:center; margin:auto;  padding:2px 0px; width:100%; border-radius:0px; ">
                               <span class="" style="color:white; font-size:100%"><span class="fa fa-shopping-cart"></span> 
                            <span id="soc{{$product->id}}">
...
function newappppc{{$product->id}}(){
        $.ajax({
            url:'https://{{$urla}}/cart_action.php?pid={{$product->id}}',
                        success:function(response){
                        $('#soc{{$product->id}}').html(response);
                        }
        });
         ...
         $.ajax({
            url:'https://{{$urla}}/cart_message.php',
                        success:function(response){
                        $('#sm11c{{$product->id}}').html(response);
        ...
        var form=document.getElementById('newloc{{$product->id}}').reset();
        return false;
...
<span id="sm11c{{$product->id}}" style="font-size:90%;margin-top:-2px"></span>
</div>
...
<section id="aa-product-category">
    <div class="container">
     ...
     <?php
      ...
      $allproducts=DB::table('products')->where('parent_id',@$ct1->id)->where('extra10',1)->orderby('id','DESC')->limit(15)->get();
      ;?>    
